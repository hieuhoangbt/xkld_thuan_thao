...
class wp_bootstrap_navwalker extends Walker_Nav_Menu
{
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        $cleanUrl = (substr($item->guid, -1, 1) == '/') ? substr($item->guid, 0, -1) : $item->guid;
        $post_id = get_the_ID();
        ...
        if ($id == $item->ID && ($depth > 0 || $depth == 0)) {
            $attributes .= ' class="active"';
        }
        $subMn = '';
        $depth == 0 ?
            $item_output = sprintf('%1$s<a%2$s>%3$s%4$s%5$s%6$s</a>%7$s', $args->before, $attributes . $collapse,
                $args->link_before, apply_filters('the_title', $item->post_title, $item->ID), $args->link_after, $caret,
                $args->after
            ) :
            $item_output = sprintf('<li>%1$s<a%2$s>%3$s%4$s%5$s</a>%6$s%7$s</li>', $args->before, $attributes,
