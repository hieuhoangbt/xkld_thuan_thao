jQuery(document).ready(function () {
    ...
    if(jQuery('#province_id').length){
        jQuery.ajax({
            ...
            if (result) {
                jQuery("#province_id").html(result);
            }
        });
    }
...
jQuery(window).load(function () {
    if (jQuery('#drug_id').length && jQuery('#drug_id').val() > 0) {
        var region_id = jQuery('#drug_id').val();
        jQuery.ajax({
            url: wpajax.ajax_url,
            type: "POST",
            data: {
                'action': 'load_city',
                'region_id': region_id,
                'active_id': jQuery("#default_province").val()
            }
        }).done(function (result) {
            if (result) {
                jQuery("#province_id").html(result);
                jQuery.ajax({
                    ...
                    data: {
                        'action': 'load_district',
                        'province_id': jQuery("#default_province").val(),
                        'active_id': jQuery("#default_district").val()
                    }
                }).done(function (result) {
                    if (result) {
                        jQuery("#district_id").html(result);
